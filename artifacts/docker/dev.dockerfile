...
COPY requirements_dev.yml /tmp/
RUN chown $UID:$GID /tmp/requirements_dev.yml
...
RUN conda update --name base --channel defaults conda && \
    conda env create --prefix /home/$USER/env --file /tmp/requirements_dev.yml --force && \
    conda clean --all --yes
...
RUN python -m ipykernel install --user --name=nerf
